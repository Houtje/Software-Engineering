...
AchievementHandler::BuilderAchievement(){
...
AchievementHandler::Cheater(QString Parent){
    QString code = Parent;
    QString match = "cheat";
...
AchievementHandler::TurnOffTheLights(){
    message = "SELECT MAX(`timestamp`) FROM `assignment_status` WHERE `accID` = " + (QString)accountID;
...
AchievementHandler::Twisted(){
     message = "SELECT EXISTS(SELECT `assID` FROM `assignment_status` WHERE `assID` < "+ (QString)assignmentID + " AND `accID` = " + (QString)accountID + " AND `score` = '')";
     query = sqlplayer->select(message);
     query.first();
...
AchievementHandler::Achiever(){
    message = "SELECT COUNT(`achID`) FROM `achievements` WHERE `accID` = " + (QString)accountID + " AND achID > 66 OR achID < 56";
    query = sqlplayer->select(message);
    query.first();
    ...
    message = "SELECT COUNT(`achID`) FROM `achievements` WHERE `accID` = " + (QString)accountID + " AND `achID` < 66 OR `achID` > 86";
    query = sqlplayer->select(message);
    query.first();
...
AchievementHandler::Flawless(){
    message = "SELECT COUNT(`flawless`) FROM `accounts` WHERE `accID` = " + (QString)accountID;
    query = sqlplayer->select(message);
    query.first();
...
AchievementHandler::TwistedAgain(){
    message = "SELECT EXISTS(SELECT * FROM `assignment_status` WHERE `score` = 1111 AND `accID` = " + (QString)accountID + ")";
    query = sqlplayer->select(message);
    query.first();
    if(query.value(0).toBool()){
        message = "SELECT EXISTS(SELECT `assID` FROM `assignment_status` WHERE `assID` < "+ (QString)assignmentID + " AND `accID` = " + (QString)accountID + " AND `score` = '')";
...
AchievementHandler::SteadyFlow(){
    ...
    int lowest;
    if(count > 5){
        message = "SELECT COUNT(`timestamp`) FROM `assignment_status` WHERE (SELECT (`timestamp` -20000 ) FROM `assignment_status` WHERE `assID` = 1 AND `accID` = 2) < `timestamp` AND (SELECT `timestamp` FROM `assignment_status` WHERE `assID` = 1 AND `accID` = 2) >= (`timestamp` - 10000) AND `accID` = 2";
        query = sqlplayer->select(message);
        query.first();
        count2 = query.value(0).toInt();
        if(count2 > 5){
            message = "SELECT COUNT(`timestamp`) FROM `assignment_status` WHERE (SELECT (`timestamp` -30000 ) FROM `assignment_status` WHERE `assID` = 1 AND `accID` = 2) < `timestamp` AND (SELECT `timestamp` FROM `assignment_status` WHERE `assID` = 1 AND `accID` = 2) >= (`timestamp` - 20000) AND `accID` = 2";
            query = sqlplayer->select(message);
            query.first();
...
bool AchievementHandler::InsertAchievement(int accountID, int achievementID ){
    message = "SELECT EXISTS(SELECT * FROM `achievements` WHERE `achID` = " + (QString)achievementID + " AND `accID` = " + (QString)accountID + ")";
    query = sqlplayer->select(message);
    query.first();
    if(!(query.value(0).toBool())){
        message = "INSERT INTO `achievements`(`achID`, `accID`) VALUES (" + (QString)achievementID + "," + (QString)accountID + ")";
...
int AchievementHandler::NmrCompletedAssignments(){
    message = "SELECT COUNT(`assID`) FROM `assignment_status` WHERE `score` = '1111' AND accID = " + (QString)accountID;
    query = sqlplayer->select(message);
    query.first();
...
//Student
AchievementHandler::SubmitStudent(int assID, QString code){
    assignmentID = assID;
    Cheater(code);
    TurnOffTheLights();
    Twisted();
...
//Docent
AchievementHandler::SubmitDocent(int assID, int accID){
    assignmentID = assID;
    accountID = accID;
    BuilderAchievement();
    Achiever();
